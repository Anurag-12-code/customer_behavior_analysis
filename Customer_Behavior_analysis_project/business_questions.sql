...
select shipping_type, ROUND(avg(purchase_amount),2) as avg_purchase from customer
...
select subscription_status,COUNT(customer_id) as total_customers, ROUND(avg(purchase_amount),2) as avg_purchase ,SUM(purchase_amount) as total_revenue from customer
...
-- 7.Segment customer into NEW, Returning, and Loyal on thier total number of previous purchases and show the count of each segment?
